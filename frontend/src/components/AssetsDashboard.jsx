...
import { RefreshCw, Building2, PlusCircle, ArrowRightLeft, Tag, FileText, IndianRupee } from 'lucide-react';
...
export default function AssetsDashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        console.error('Failed to load assets dashboard', err);
      }
  ...
  if (!stats) return <div style={{ color: 'var(--text-muted)' }}>Unable to load assets dashboard.</div>;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: '12px', flexWrap: 'nowrap', width: '100%' }}>
        <KPICard icon={Building2}       label="Total Assets"   value={stats.totalAssets || 0} />
        <KPICard icon={PlusCircle}      label="Additions"      value={stats.assetAdditions || 0} />
        <KPICard icon={ArrowRightLeft}  label="Allocations"    value={stats.assetAllocations || 0} />
        <KPICard icon={Tag}             label="Sales"          value={stats.assetSales || 0} />
        <KPICard icon={FileText}        label="Dep. Vouchers"  value={stats.depreciationVouchers || 0} />
      </div>

      {/* Row 1: Line + Pie */}
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '16px' }}>
        <div className="card" style={{ padding: '16px' }}>
          <h3 style={{ marginBottom: '12px', color: '#374151', fontSize: '14px', fontWeight: 600 }}>Depreciation by Year</h3>
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={chartMap['assets-dep'] || []}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" vertical={false} />
              <XAxis dataKey="name" tick={{ fontSize: 10 }} axisLine={false} tickLine={false} />
              <YAxis tickFormatter={formatLakh} tick={{ fontSize: 10 }} axisLine={false} tickLine={false} width={55} />
              <Tooltip content={<CustomTooltip />} />
              <Line dataKey="value" name="Depreciation" stroke="#F97316" strokeWidth={3} dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>

        <div className="card" style={{ padding: '16px' }}>
          <h3 style={{ marginBottom: '12px', color: '#374151', fontSize: '14px', fontWeight: 600 }}>Asset Group Mix</h3>
          <ResponsiveContainer width="100%" height={220}>
            <PieChart margin={{ top: 10, right: 40, left: 40, bottom: 10 }}>
              <Pie
                data={chartMap['assets-group'] || []}
                dataKey="value" nameKey="name"
                cx="50%" cy="50%"
                innerRadius={35} outerRadius={55}
                paddingAngle={2}
              >
                {(chartMap['assets-group'] || []).map((entry, idx) => (
      ...
      {/* Row 2: Status bar + Recent activity */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '16px' }}>
        <div className="card" style={{ padding: '16px' }}>
          <h3 style={{ marginBottom: '12px', color: '#374151', fontSize: '14px', fontWeight: 600 }}>Asset Status</h3>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={chartMap['assets-status'] || []} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" vertical={false} />
              <XAxis dataKey="name" tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 10 }} axisLine={false} tickLine={false} />
              <Tooltip formatter={(v) => Number(v || 0)} />
              <Bar dataKey="value" name="Count" radius={[4, 4, 0, 0]}>
                {(chartMap['assets-status'] || []).map((_, idx) => (
        ...
        <div className="card" style={{ padding: '16px' }}>
          <h3 style={{ marginBottom: '12px', color: '#374151', fontSize: '14px', fontWeight: 600 }}>Recent Asset Activity</h3>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid #e5e7eb' }}>
                  <th style={{ textAlign: 'left', padding: '10px 8px', fontSize: '12px', fontWeight: 600, color: '#6B7280' }}>Type</th>
                  <th style={{ textAlign: 'left', padding: '10px 8px', fontSize: '12px', fontWeight: 600, color: '#6B7280' }}>Asset</th>
                  ...
                  <tr key={`${row.id}-${idx}`} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    <td style={{ padding: '8px' }}>
                      <span style={{
                        ...
                        background: row.type === 'Sale Memo' ? '#FED7AA' : row.type === 'Addition' ? '#E5E7EB' : '#FFEDD5',
                        color: row.type === 'Sale Memo' ? '#C2410C' : row.type === 'Addition' ? '#374151' : '#EA580C',
                      }}>{row.type}</span>
                    </td>
