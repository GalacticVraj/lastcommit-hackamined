...
import { RefreshCw, Users, FileText, IndianRupee, TrendingUp, CreditCard } from 'lucide-react';
...
export default function ContractorsDashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        console.error('Failed to load contractors dashboard', err);
      }
  ...
  if (!stats) return <div style={{ color: 'var(--text-muted)' }}>Unable to load contractors dashboard.</div>;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: '12px', flexWrap: 'nowrap', width: '100%' }}>
        <KPICard icon={Users}       label="Total Workers"  value={stats.totalWorkers || 0} />
        <KPICard icon={FileText}    label="Salary Heads"   value={stats.salaryHeads || 0} />
        <KPICard icon={FileText}    label="Salary Sheets"  value={stats.salarySheets || 0} />
        <KPICard icon={IndianRupee} label="Total Payable"  value={stats.totalPayable || 0} isCurrency />
        <KPICard icon={CreditCard}  label="Total Advances" value={stats.totalAdvances || 0} isCurrency />
      </div>

      {/* Row 1: Line + Pie */}
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '16px' }}>
        <div className="card" style={{ padding: '16px' }}>
          <h3 style={{ marginBottom: '12px', color: '#374151', fontSize: '14px', fontWeight: 600 }}>Monthly Payable Trend</h3>
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={chartMap['contractors-payable'] || []}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" vertical={false} />
              <XAxis dataKey="name" tick={{ fontSize: 10 }} axisLine={false} tickLine={false} />
              <YAxis tickFormatter={formatLakh} tick={{ fontSize: 10 }} axisLine={false} tickLine={false} width={55} />
              <Tooltip content={<CustomTooltip />} />
              <Line dataKey="value" name="Payable" stroke="#F97316" strokeWidth={3} dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>

        <div className="card" style={{ padding: '16px' }}>
          <h3 style={{ marginBottom: '12px', color: '#374151', fontSize: '14px', fontWeight: 600 }}>Worker Skill Mix</h3>
          <ResponsiveContainer width="100%" height={220}>
            <PieChart margin={{ top: 10, right: 40, left: 40, bottom: 10 }}>
              <Pie
                data={chartMap['contractors-skill'] || []}
                dataKey="value" nameKey="name"
                cx="50%" cy="50%"
                innerRadius={35} outerRadius={55}
              >
                {(chartMap['contractors-skill'] || []).map((entry, idx) => (
      ...
      {/* Row 2: Advance vs Payout bar + Recent table */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '16px' }}>
        <div className="card" style={{ padding: '16px' }}>
          <h3 style={{ marginBottom: '12px', color: '#374151', fontSize: '14px', fontWeight: 600 }}>Advance vs Voucher Payout</h3>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={chartMap['contractors-cashflow'] || []} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" vertical={false} />
              <XAxis dataKey="name" tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
              <YAxis tickFormatter={formatLakh} tick={{ fontSize: 10 }} axisLine={false} tickLine={false} width={55} />
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey="value" name="Amount" radius={[4, 4, 0, 0]}>
                {(chartMap['contractors-cashflow'] || []).map((_, idx) => (
        ...
        <div className="card" style={{ padding: '16px' }}>
          <h3 style={{ marginBottom: '12px', color: '#374151', fontSize: '14px', fontWeight: 600 }}>Recent Salary & Voucher Activity</h3>
                  ...
                  <tr key={`${row.id}-${idx}`} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    <td style={{ padding: '8px' }}>
                      <span style={{
                        ...
                        background: row.type === 'Salary Sheet' ? '#FFEDD5' : '#E5E7EB',
                        color: row.type === 'Salary Sheet' ? '#EA580C' : '#374151',
                      }}>{row.type}</span>
                    </td>
