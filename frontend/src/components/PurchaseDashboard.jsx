...
import { RefreshCw, Users, FileText, Truck, Receipt } from 'lucide-react';
      ...
      <div style={{
        ...
        {payload.map((entry, idx) => (
          <p key={idx} style={{ color: entry.color, margin: '3px 0', fontSize: '11px' }}>
            {entry.name}: {entry.name?.toLowerCase().includes('spend') || entry.name?.toLowerCase().includes('amount') ? formatLakh(Number(entry.value || 0)) : Number(entry.value || 0).toLocaleString('en-IN')}
          </p>
        ))}
      </div>
    );
...
export default function PurchaseDashboard() {
  ...
  if (!stats) return <div style={{ color: 'var(--text-muted)' }}>Unable to load purchase dashboard.</div>;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', gap: '12px', flexWrap: 'nowrap', width: '100%' }}>
        <KPICard icon={Users} label="Vendors" value={stats.totalVendors || 0} />
        <KPICard icon={FileText} label="POs" value={stats.totalPOs || 0} />
        <KPICard icon={Truck} label="Open POs" value={stats.pendingGRNs || 0} />
        <KPICard icon={Receipt} label="Pending GRNs" value={stats.pendingBills || 0} />
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '16px' }}>
        <div className="card" style={{ padding: '16px' }}>
          <h3 style={{ marginBottom: '12px', color: '#374151', fontSize: '14px', fontWeight: 600 }}>PO Trend by Month</h3>
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={chartMap['purchase-monthly'] || []}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" vertical={false} />
              <XAxis dataKey="name" tick={{ fontSize: 10 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 10 }} axisLine={false} tickLine={false} width={40} />
              <Tooltip content={<CustomTooltip />} />
              <Line dataKey="value" name="PO Count" stroke="#F97316" strokeWidth={3} dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>

        <div className="card" style={{ padding: '16px' }}>
          <h3 style={{ marginBottom: '12px', color: '#374151', fontSize: '14px', fontWeight: 600 }}>PO Status Mix</h3>
          <ResponsiveContainer width="100%" height={220}>
            <PieChart margin={{ top: 10, right: 40, left: 40, bottom: 10 }}>
              <Pie data={chartMap['purchase-status'] || []} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={35} outerRadius={55}>
                {(chartMap['purchase-status'] || []).map((entry, idx) => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '16px' }}>
        <div className="card" style={{ padding: '16px' }}>
          <h3 style={{ marginBottom: '12px', color: '#374151', fontSize: '14px', fontWeight: 600 }}>Top Vendor Spend</h3>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={chartMap['purchase-vendor-spend'] || []} layout="vertical" margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" horizontal={false} />
              <XAxis type="number" tick={{ fontSize: 10 }} axisLine={false} tickLine={false} />
              <YAxis dataKey="name" type="category" tick={{ fontSize: 11 }} axisLine={false} tickLine={false} width={90} />
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey="value" name="Spend" radius={[0, 4, 4, 0]} fill="#F97316" />
            </BarChart>
          </ResponsiveContainer>
        </div>

        <div className="card" style={{ padding: '16px' }}>
          <h3 style={{ marginBottom: '12px', color: '#374151', fontSize: '14px', fontWeight: 600 }}>Recent Purchase Activity</h3>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid #e5e7eb' }}>
                  ...
                  <th style={{ textAlign: 'left', padding: '10px 8px', fontSize: '12px', fontWeight: 600, color: '#6B7280' }}>Vendor</th>
              ...
              <tbody>
                {recent.slice(0, 8).map((row, idx) => (
                  <tr key={`${row.type}-${row.id}-${idx}`} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    ...
                    <td style={{ padding: '8px', fontSize: '12px', color: '#374151' }}>{row.party || '-'}</td>
